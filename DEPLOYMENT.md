...
```bash
docker build -t loyalty-rewards-service:local .
docker run --rm -p 8080:8080 loyalty-rewards-service:local
```

## Manual Cloud Run Deploy

```bash
IMAGE=us-central1-docker.pkg.dev/probestack-prod/probestack-prod-apps/loyalty-rewards-service:manual

docker build -t ${IMAGE} .
docker push ${IMAGE}

gcloud run deploy loyalty-rewards-service \
  --image=${IMAGE} \
  --platform=managed \
  --region=us-central1 \
